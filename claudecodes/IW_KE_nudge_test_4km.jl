#= IW_KE_200m_2000km_bash_cuda.jl
...
if runmode == "batch"
    ...
    if length(ARGS) != 6
        error("Usage: julia IW_Amz2_cuda.jl <mainnm> <runnm> <lat> <numM> <Usur1> <Usur2>\n" *
              ...
              "  KE1    : mode-1 depth-integrated KE [J/m2]\n" *
              "  KE2    : mode-2 depth-integrated KE [J/m2]")
    end
    ...
    KEtarget1 = parse(Float64, ARGS[5])   # mode-1 depth-integrated KE [J/m2]
    KEtarget2 = parse(Float64, ARGS[6])   # mode-2 depth-integrated KE [J/m2]

elseif runmode == "manual"

    # ---- MANUAL MODE PARAMETERS: edit these directly for debugging ----
    mainnm    = 3           # 4 km test series
    runnm     = 99          # scratch slot
    lat       = 40.0        # stress latitude (over-driving was worst here)
    numM      = [1]         # e.g. [1] or [1,2]
    KEtarget1 = 12_000.0    # mode-1 depth-integrated time-mean KE [J/m2]
    KEtarget2 = 3_000.0     # mode-2 depth-integrated time-mean KE [J/m2]
    # ---------------------------------------------------------------
...
println("runmode = $runmode, mainnm = $mainnm, runnm = $runnm, lat = $lat, numM = $numM, KEtarget1 = $KEtarget1, KEtarget2 = $KEtarget2")
...
DX  = 4000;                        # 4 km TEST grid (fast); production is 200
#DX  = 100;
max_Δt = 2minutes  
Δt     = 15seconds   # nonhyd
...
# --- KE-target -> modal amplitude a_m ----------------------------------------
# depth-integrated, time-mean horizontal KE:
#   KE = 1/4 * rho0 * a^2 * (1 + f^2/w^2) * ∫Ueig2^2 dz
# invert for the modal amplitude a_m that hits KEtarget_m (Ueig2: ⟨Φ²⟩=1, ∫≈H)
KEtarget = (KEtarget1, KEtarget2)
dzf      = diff(zfw)
polar    = 1 + (pm.f / pm.ω)^2
a_modal  = zeros(nModes)
for i in 1:nModes
    Iphi       = sum(Ueig2[:, i].^2 .* dzf)      # ∫Ueig2_i^2 dz  (≈ H)
    a_modal[i] = sqrt(4*KEtarget[i] / (rho0 * polar * Iphi))
    @printf("mode %d: KEtarget=%8.1f J/m2 -> a=%.5f, u0_surface=%.4f m/s (∫Ueig2²dz=%.1f, H=%.0f)\n",
            i, KEtarget[i], a_modal[i], a_modal[i]*Ueig2[end, i], Iphi, H)
end
