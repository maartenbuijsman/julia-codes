...
# b_sponge relaxes toward background buoyancy, not zero
@inline b_sponge(x, z, t, b, p) = -(fnudl * left_mask(x, p) + fnudr * right_mask(x, p)) * (b - gpu_interp1(p.zfw, p.bb, z))
...
model = HydrostaticFreeSurfaceModel(grid;
    ...
    closure            = ScalarDiffusivity(VerticallyImplicitTimeDiscretization(), ν=1e-5, κ=1e-5),
    tracers            = :b,
    buoyancy           = BuoyancyTracer(),
    free_surface       = ImplicitFreeSurface(),
    forcing            = (u=u_forcing, v=v_forcing, b=b_forcing))

# initialize b with background buoyancy profile
bᵢ(x, z) = linear_interpolation(zfw, bb_cpu, extrapolation_bc=Interpolations.Flat())(z)
set!(model, b = bᵢ)
