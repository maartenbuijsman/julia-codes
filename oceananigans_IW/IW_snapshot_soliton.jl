...
function run_figs(mainnm,runnm,LAT,titstr,figcount)
...
xlim = (300,400); zlim = (-1500,0); fstr = "zoom_u_rho_";
...
ax = Axis(fig[figcount, 1],title=titstr, xlabel="x [km]", ylabel="z [m]")#, xticks = 367:2:387)
...
elapsed = @elapsed begin
    ...
    for (mainnm,runnm,LAT,titstr) in zip(mainnms,runnms,LATS,titstrs)
        figcount = figcount + 1;
        run_figs(mainnm,runnm,LAT,titstr,figcount)
    end 
end
