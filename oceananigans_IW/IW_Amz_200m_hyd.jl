#= IW_Amz_200m_hyd.jl
Maarten Buijsman, USM DMS, 2026-2-6
...
@inline force_u(x, z, t, u, p) = Fu_wave(x, z, t, p)
@inline force_v(x, z, t, v, p) = 0 #+ Fv_wave(x, z, t, p) 
@inline force_w(x, z, t, w, p) = 0 #+ Fw_wave(x, z, t, p)
@inline force_b(x, z, t, b, p) = 0
...
model = HydrostaticFreeSurfaceModel(; grid, coriolis=fcor,
                momentum_advection = WENO(),
                tracer_advection = WENO(),
                tracers = :b,
                buoyancy = BuoyancyTracer(),
                forcing = (u=u_forcing, v=v_forcing, w=w_forcing)) # need to include restoring              
#                forcing = (u=u_forcing, v=v_forcing, w=w_forcing, b=b_forcing)) 

# set initial condition
# for buoyancy and velocity

bi(x, z) = B_func(x, z, 0, pm)
set!(model, u=0.0, b=bi)
