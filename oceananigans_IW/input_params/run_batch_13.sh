#!/bin/bash
# ============================================================
# run_batch.sh
#
# Reads params*.jl, then loops over the (lat, runnm) pairs,
# launching a fresh Julia process for each one so that the GPU
# is fully released between runs. 
#
# Usage:
#   chmod +x run_batch.sh
#   ./run_batch.sh [params_file]          # defaults to params.jl
#
# SLURM example (one node, one GPU):
#   sbatch run_batch.sh
# ============================================================

#SBATCH --job-name=IW_Amz3_GM 
#SBATCH --output=logs/IW_Amz3_GM_%j.log
#SBATCH --error=logs/IW_Amz3_GM_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --gres=gpu:1
#SBATCH --time=48:00:00
#SBATCH --partition=gpu

# ---- configuration ----------------------------------------
MAINNM="13"
JULIA_SCRIPT="IW_GM_flux_LAT_2000km_bash_cuda.jl"      # JL script name
JULIA_PTH="/home/mbui/Documents/julia-codes/oceananigans_IW"      # path of .jl file
JULIA_PTHFNM="${JULIA_PTH}/${JULIA_SCRIPT}"   # path to simulation script
JULIA_BIN="${JULIA:-julia}"                   # honour $JULIA env var if set
JULIA_THREADS="${JULIA_THREADS:-$(nproc)}"    # number of Julia threads (auto-detects number of available cores)
#JULIA_THREADS="${JULIA_THREADS:-4}"           # number of Julia threads
PARAMS_FILE="${1:-params_${MAINNM}.jl}"       # first argument or default
LOG_DIR="logs"
# -----------------------------------------------------------

mkdir -p "$LOG_DIR"

echo "=================================================="
echo "Batch run started: $(date)"
echo "Parameter file   : $PARAMS_FILE"
echo "Julia script     : $JULIA_PTHFNM"
echo "Julia binary     : $JULIA_BIN"
echo "Julia threads    : $JULIA_THREADS"
echo "=================================================="

# ---- parse params.jl with Julia ---------------------------
# Emit one line per run:  <mainnm> <runnm> <lat> <numM> <Usur1> <Usur2>
PARAM_LINES=$(
$JULIA_BIN --startup-file=no -e "
include(\"$PARAMS_FILE\")
for (r, l, nm, u1, u2) in zip(runnm, lat, numM, Usur1, Usur2)
    println(mainnm, \" \", r, \" \", l, \" \", nm, \" \", u1, \" \", u2)
end
"
)

if [ -z "$PARAM_LINES" ]; then
    echo "ERROR: could not parse $PARAMS_FILE – exiting."
    exit 1
fi

NTOTAL=$(echo "$PARAM_LINES" | wc -l)
echo "Total runs to execute: $NTOTAL"
echo ""

# ---- main loop --------------------------------------------
RUN_IDX=0
while IFS=' ' read -r MAINNM RUNNM LAT NUMM USUR1 USUR2; do
    RUN_IDX=$((RUN_IDX + 1))
    RUNLOG="${LOG_DIR}/run_${MAINNM}_${RUNNM}_lat${LAT}_numM${NUMM}.log"

    echo "--------------------------------------------------"
    echo "Run ${RUN_IDX}/${NTOTAL}  |  mainnm=${MAINNM}  runnm=${RUNNM}  lat=${LAT}  numM=${NUMM}  Usur1=${USUR1}  Usur2=${USUR2}"
    echo "Log: $RUNLOG"
    echo "Start: $(date)"

    # Launch Julia, wait for it to finish, then continue.
    # Each invocation is a completely fresh process – GPU memory
    # is freed when Julia exits before the next run begins.
    $JULIA_BIN \
        --startup-file=no \
        --threads=$JULIA_THREADS \
        "$JULIA_PTHFNM" \
        "$MAINNM" "$RUNNM" "$LAT" "$NUMM" "$USUR1" "$USUR2" \
        2>&1 | tee "$RUNLOG"

    EXIT_CODE=${PIPESTATUS[0]}

    if [ $EXIT_CODE -ne 0 ]; then
        echo "WARNING: Julia exited with code $EXIT_CODE for run ${MAINNM}.${RUNNM} (lat=${LAT})."
        echo "Check $RUNLOG fo
r details."
        # Change 'continue' to 'exit 1' if you want the whole batch to stop on failure.
        continue
    fi

    echo "Finished run ${RUN_IDX}/${NTOTAL}  |  $(date)"

done <<< "$PARAM_LINES"

echo ""
echo "=================================================="
echo "All $NTOTAL runs completed: $(date)"
echo "=================================================="
