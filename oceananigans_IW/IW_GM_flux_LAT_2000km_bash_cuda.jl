...
Maarten Buijsman, USM DMS, 2026-9-2  (nudging generator, generated with Claude Code)
    ...
    initial condition for u,v (see claudecodes/GM_spectrum_init_2D.jl for the
    full derivation/assumptions) applied via set!(model, u=u_gm, v=v_gm)
    right after model creation.
...
#= Builds u_gm(x,y,z), v_gm(x,y,z) for set!(model, u=u_gm, v=v_gm) below.
...
const Egm_gm  = 6.3e-5      # GM76 energy parameter
const js_gm   = 3.0         # mode-number scale j*
const N0_gm   = 5.24e-3     # reference buoyancy frequency [rad/s]
const bgm_gm  = 1300.0      # stratification e-folding scale [m]
const jsum_gm = (π*js_gm/tanh(π*js_gm) - 1)/(2*js_gm^2)
...
for n in 1:gm_Nfreq
    om = omg_mid_gm[n]
    k_gm_n, Lw_gm_n, _, _, _, _, _, Ueig2_gm_n =
        sturm_liouville_noneqDZ_norm(zfw_gm, N2w_gm, f_gm, om, nonhyd)
    for j in 1:length(k_gm_n)
        Lw_gm_n[j] < gm_minwavelenfac*DX && continue   # unresolved on the grid
        Lw_gm_n[j] > L && continue                      # longer than the domain
        K = Komgj_gm(om, j, f_gm)
        A = sqrt(2 * K * domg_gm[n])
...
println("setting GM-spectrum initial condition for u,v ...")
set!(model, u=u_gm, v=v_gm)
